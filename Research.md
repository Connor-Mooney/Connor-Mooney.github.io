...
At slow-enough timescales, quantum dynamics starting in the ground state are rigorously governed by the adiabatic theorem, which guarantees approximate spectral transport, with error (heuristically) scaling like {% katex %}\sim(\Gamma^2 T)^{-1}{% endkatex %} where {% katex %}\Gamma{% endkatex %} is the spectral gap. These guarantees have been harnessed 
